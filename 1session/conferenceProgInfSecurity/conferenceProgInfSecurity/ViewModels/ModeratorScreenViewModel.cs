...
namespace conferenceProgInfSecurity.ViewModels
{
    ...
    public class ModeratorScreenViewModel : ViewModelBase
    {
        ...
        private void LoadModeratorData(Moderator moderator)
        {
            ...
            string moderatorName = !string.IsNullOrWhiteSpace(moderator.Namemoderator) ? moderator.Namemoderator : "Имя не указано";

            // Получаем отчество организатора (если не указано — подставляется пустая строка)
            string moderatorPatronymic = !string.IsNullOrWhiteSpace(moderator.Patronymicmoderator) ? moderator.Patronymicmoderator : "";

            // Формируем полное имя с отчеством (если отчество есть)
            string fullName = string.IsNullOrWhiteSpace(moderatorPatronymic)
                ? moderatorName
                : $"{moderatorName} {moderatorPatronymic}";
