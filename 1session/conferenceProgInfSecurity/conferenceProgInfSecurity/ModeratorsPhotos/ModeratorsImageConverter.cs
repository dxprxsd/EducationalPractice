...
namespace conferenceProgInfSecurity.ModeratorsPhotos
{
    internal class ModeratorsImageConverter : IValueConverter
    {
        public object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
        {
            ...
            string ModeratorsPhotosPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ModeratorsPhotos");

            // Проверка, если значение пустое
            if (value == null || string.IsNullOrWhiteSpace(value.ToString()))
            {
                // Путь к изображению-заглушке
                string defaultImagePath = Path.Combine(ModeratorsPhotosPath, "foto1.jpg");
            ...
            if (value is string fileName)
            {
                ...
                string imagePath = Path.Combine(ModeratorsPhotosPath, fileName.Trim());
