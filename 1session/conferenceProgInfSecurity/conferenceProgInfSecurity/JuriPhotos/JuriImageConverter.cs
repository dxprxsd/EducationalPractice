...
namespace conferenceProgInfSecurity.JuriPhotos
{
    internal class JuriImageConverter : IValueConverter
    {
        public object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
        {
            ...
            string JuriPhotosPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "JuriPhotos");

            // Проверка, если значение пустое
            if (value == null || string.IsNullOrWhiteSpace(value.ToString()))
            {
                // Путь к изображению-заглушке
                string defaultImagePath = Path.Combine(JuriPhotosPath, "foto1.jpg");
            ...
            if (value is string fileName)
            {
                ...
                string imagePath = Path.Combine(JuriPhotosPath, fileName.Trim());
